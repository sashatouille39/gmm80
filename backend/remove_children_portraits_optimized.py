"""
Script optimisé pour identifier et supprimer les portraits d'enfants (âge < 21 ans)
Version légère en mémoire avec traitement par batch
"""
import os
import json
import shutil
...
# Limiter l'utilisation mémoire AVANT d'importer TensorFlow
...
BATCH_SIZE = 50  # Traiter par lots de 50 images
SAVE_INTERVAL = 100  # Sauvegarder le rapport tous les 100 images
...
class ChildrenPortraitRemover:
    def __init__(self):
        ...
        # Charger le rapport existant si présent (pour reprendre après un crash)
        self.load_existing_report()
    
    def load_existing_report(self):
        """Charge le rapport existant pour reprendre après un crash"""
        if self.report_file.exists():
            try:
                with open(self.report_file, 'r', encoding='utf-8') as f:
                    report = json.load(f)
                    self.removed_portraits = report.get('removed_portraits', [])
                    self.scanned_count = report.get('summary', {}).get('total_scanned', 0)
                    self.error_count = report.get('summary', {}).get('errors', 0)
                    print(f"📂 Rapport existant chargé: {len(self.removed_portraits)} portraits déjà traités")
            except:
                pass
    ...
    def check_age(self, image_path: Path) -> dict:
        ...
        try:
            # Analyser avec DeepFace (backend léger)
            analysis = DeepFace.analyze(
                img_path=str(image_path),
                actions=['age'],
                enforce_detection=False,
                detector_backend='opencv',  # Le plus léger
                silent=True
            )
    ...
    def scan_and_remove(self):
        ...
        print(f"🔧 Traitement par batch de {BATCH_SIZE} images\n")
        ...
        for folder in PORTRAIT_FOLDERS:
            ...
            for i, image_path in enumerate(tqdm(image_files, desc=f"  {folder}")):
                ...
                # Libérer la mémoire tous les BATCH_SIZE images
                if batch_count >= BATCH_SIZE:
                    gc.collect()
                    batch_count = 0
                
                # Sauvegarder la progression
                if self.scanned_count % SAVE_INTERVAL == 0:
                    self.save_progress()
            
            if removed_in_folder > 0:
                print(f"  ❌ Supprimés: {removed_in_folder} portraits d'enfants")
            
            # Libérer la mémoire entre les dossiers
            gc.collect()
    
    def generate_report(self):
        """Génère un rapport détaillé final"""
        self.save_progress()
...
def main():
    print("\n" + "="*60)
    print("🔞 SUPPRESSION DES PORTRAITS D'ENFANTS (VERSION OPTIMISÉE)")
    print("="*60)
